...
After fifteen years in engineering management, I've discovered that the principles I learned managing humans at Netflix apply remarkably well to managing AI agents. The philosophy behind Netflix engineering culture, "context, not control," turns out to be exactly what's needed to get large productivity gains from frontier AI models.

The goal: extract maximum productivity from AI. If you want maximum control, you can always write the code yourself. But if you want to compress weeks of work into hours, you need to master context management for these alien intelligences that think differently than we do.
...
The answer surprises managers used to detailed instructions and oversight. Technical work is creative work, and creativity thrives with freedom. The best environment for innovation gave talented people the context to make good decisions and the freedom to execute them.
...
Today, I'm seeing these same patterns emerge in AI-assisted coding. Frontier AI models deliver large productivity gains. I've watched project timelines compress from weeks to days to hours. But these models are costly, and getting value from them requires mastering context management all over again.

Just like those Netflix engineers, AI agents do their best work when given freedom within well-defined context. The challenge is providing the right context so they make good decisions autonomously.
...
Good context management starts with understanding what context actually means for AI. You need to provide enough information for good decisions without drowning the model in irrelevant details.

Sometimes it's more efficient to use dialogue for context alignment rather than typing everything out. You can use the AI's existing knowledge, for example asking it to explain key components back to you as a trust-but-verify mechanism. That back-and-forth surfaces assumptions and gaps that static documentation misses.

To gain productivity from context alignment, you must be detailed in your context review. Vague context leads to vague implementations.

The payoff comes from translating business requirements into concrete engineering decisions. Instead of saying "this needs to scale," you provide engineering choices that achieve scaling goals. Is this a weekend project or an enterprise deployment? What's the maximum number of users? How will traffic spike? These questions translate into concrete decisions: monolithic versus distributed architecture, specific test coverage goals, failsafe mechanisms, database choices based on consistency versus availability tradeoffs.
...
I've caught many off-the-rails failures by simply staying engaged with the output instead of context-switching to other work. This vigilance is particularly important when working with a new AI model, on system components, on hard architectural decisions, or on ambiguous requirements.
...
One unique advantage of AI-assisted development is the ability to treat failed attempts as learning experiences with minimal cost. If a project goes off track, you have an option you never had with human developers: `git restore` and start over.

You're simulating best-of-N sampling, an ML technique where you generate multiple attempts and pick the best one. Each fresh start is another sample, another chance at a better outcome. And you're honing your context provision skills. Each attempt refines your understanding of what context the AI needs. The second attempt incorporates lessons from the first failure. The third attempt might nail it perfectly. The ability to reset and retry with improved context turns every mistake into a learning opportunity.
...
Engineers report 10x, sometimes 100x improvements in their output. They're building systems that would have taken teams months, now done by one person in days.
...
AI thinks differently than we do. It has alien strengths and alien weaknesses. Master context management, and you can direct these strange intelligences toward productive ends.
