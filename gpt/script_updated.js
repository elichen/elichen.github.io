...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    generateButton.addEventListener('click', async () => {
        ...
        try {
            const prompts = [
                { text: 'The ', temp: 0.8 },
                { text: 'To be or ', temp: 0.7 },
                { text: 'What ', temp: 0.8 },
                { text: 'In the ', temp: 0.8 }
            ];
            
            let outputHTML = '<h3>Generated Text Samples:</h3>';
            
            for (const { text, temp } of prompts) {
                const generated = await gptModel.generateText(text, 150, temp);
                outputHTML += `
                    <div style="margin: 15px 0; padding: 10px; background: #f9f9f9; border-left: 3px solid #4CAF50;">
                        <strong>Prompt:</strong> "${text}" (temperature: ${temp})<br>
