...
class AirHockeyEnv(gym.Env):
    ...
    def step(self, action):
        ...
        reward = 0.0
        if goal_scored_by == 1: reward = 1.0  # Score goal
        elif goal_scored_by == 2: reward = -1.0  # Receive goal
        if self.frame_count >= self.max_frames: reward = -0.5  # Fault
    ...
    def _get_observation(self, player):
        ...
        opp_rel = (opponent_pos - paddle_pos) / [self.width, self.height]
        ...
        return np.array([puck_rel[0], puck_rel[1], puck_v[0], puck_v[1], opp_rel[0], opp_rel[1],
                        dist, angle, behind, goal_dist, puck_goal_dist, paddle_v[0], paddle_v[1], puck_spd], dtype=np.float32)
    ...
    def _update_puck(self):
        ...
        if self.puck_pos[0] <= self.puck_radius:
            self.puck_pos[0] = self.puck_radius
            self.puck_vel[0] = abs(self.puck_vel[0]) * 0.9
        elif self.puck_pos[0] >= self.width - self.puck_radius:
            self.puck_pos[0] = self.width - self.puck_radius
            self.puck_vel[0] = -abs(self.puck_vel[0]) * 0.9

        if self.puck_pos[1] <= self.puck_radius:
            self.puck_pos[1] = self.puck_radius
            self.puck_vel[1] = abs(self.puck_vel[1]) * 0.9
        elif self.puck_pos[1] >= self.height - self.puck_radius:
    ...
    def _check_paddle_collision(self, paddle_pos, paddle_vel):
        ...
        if dist < min_dist and dist > 0:
            ...
            self.puck_pos += normal * (min_dist - dist)
            vel_along = np.dot(self.puck_vel - paddle_vel, normal)
            if vel_along < 0:
                self.puck_vel -= 2 * vel_along * normal * 0.8
                self.puck_vel += paddle_vel * 0.3

    def _check_goals(self):
        goal_x_min, goal_x_max = (self.width - self.goal_width)/2, (self.width + self.goal_width)/2
        if self.puck_pos[1] <= self.puck_radius and goal_x_min <= self.puck_pos[0] <= goal_x_max: return 1
        if self.puck_pos[1] >= self.height - self.puck_radius and goal_x_min <= self.puck_pos[0] <= goal_x_max: return 2
        return 0
